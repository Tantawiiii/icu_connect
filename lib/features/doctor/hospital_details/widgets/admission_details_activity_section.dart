...
class _ActivityTile extends StatelessWidget {
  ...
  String _title(AdmissionActivity activity) {
    final typeLabel = activity.subjectTypeEnum?.label ??
        (activity.subjectType.isEmpty
            ? AppTexts.activityHistorySection
            : activity.subjectType.replaceAll('_', ' '));
    final event = activity.event.trim();
    if (event.isEmpty) return typeLabel;
    return '$typeLabel · ${event[0].toUpperCase()}${event.substring(1)}';
  }
